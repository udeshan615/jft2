...
import { KanjiGamesAdmin } from '@/components/admin/kanji-games-admin';
...
export default async function AdminKanjiBooksPage() {
  ...
  const [{ data: books }, { data: lessons }, { data: entries }, { data: games }] =
    await Promise.all([
      supabase
        .from('content_collections')
        .select('id, title, description, book_number, sort_order, status, metadata')
        .eq('kind', 'kanji_book')
      ...
      supabase
        ...
        .order('sort_order', { ascending: true }),
      supabase.from('kanji_games').select('*').order('sort_order', { ascending: true }),
    ]);
  ...
  return (
    <>
      <KanjiBooksAdmin
        ...
        adminId={admin.id}
      />
      <KanjiGamesAdmin books={books ?? []} games={games ?? []} />
    </>
  );
}
